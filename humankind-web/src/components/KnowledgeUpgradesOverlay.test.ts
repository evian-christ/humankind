...
describe('knowledge upgrades', () => {
    it('exposes only upgrades with active effects', () => {
        ...
        expect(KNOWLEDGE_UPGRADE_TRACKS).toHaveLength(5);
    });
    ...
    it('derives upgrade levels from legacy unlocked upgrade IDs', () => {
        ...
        expect(levels.pastoralism).toBe(1);
        expect(levels.trade).toBe(1);
        expect(levels.faith).toBe(0);
    });

    it('returns only the next effect stage of an upgrade', () => {
        const levels = deriveKnowledgeUpgradeLevels([]);
        expect(getNextKnowledgeUpgradeTrackStage('trade', levels)?.upgradeId).toBe(GUILD_UPGRADE_ID);
    });

    it('rebuilds all previous effect IDs from an upgrade level', () => {
        const levels = deriveKnowledgeUpgradeLevels([EDUCATION_UPGRADE_ID]);
        expect(getUnlockedUpgradeIdsForKnowledgeLevels(levels)).toEqual(
            expect.arrayContaining([EDUCATION_UPGRADE_ID]),
        );
    });
});
