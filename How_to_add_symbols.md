...
심볼 정의의 `tags` 필드는 **유저가 명시적으로 요청한 태그만** 추가한다.

- 유저가 "food 태그 달아줘"라고 하지 않았다면, `tags: ["food"]`를 임의로 추가하지 않는다.
- 태그가 없을 때는 `tags: []`로 비워둔다.

> ❌ 잘못된 예: 식량을 생산한다고 해서 임의로 `tags: ["food"]` 추가
> ✅ 올바른 예: 유저가 요청하지 않으면 `tags: []`

**왜인가**: 태그는 상점 필터링, 시너지 조건, 업그레이드 해금 등 게임 로직에 영향을 줄 수 있다. 임의 태그는 예상치 못한 상호작용을 유발한다.
...
1. [ ] `symbolDefinitions.ts`에 ID, 이름, 시대, 타입, 설명, 스프라이트, 태그 추가
2. [ ] `sprite` 필드: `public/assets/symbols/`에서 파일 존재 확인 → 없으면 `"-"`
3. [ ] `tags` 필드: 유저 요청 태그만, 없으면 `[]`
