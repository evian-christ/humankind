...
- 스핀당 이터레이션 순서: **전투 페이즈 → 효과 페이즈**.
  - 전투 페이즈: 전투 심볼이 인접 적 심볼을 공격. 적 심볼이 인접 아군 심볼에 반격 피해 적용.
  - 효과 페이즈: 살아남은 심볼들이 식량·골드·지식 생산 (현재 구현된 흐름).
- 전쟁은 플레이어의 선택이지만, 야만인 출현은 강제 → 기본 방어는 필수.
- **전투 심볼 배치 우선순위**: 아군 전투 심볼·적 심볼 모두 스핀 시 일반 심볼보다 보드 배치 우선순위가 높음. 컬렉션에 전투 심볼이 존재하면 거의 항상 보드에 배치됨. 전투가 매 스핀 일어나도록 보장하기 위한 구조.
...
|------|------|----------|------------|--------------|
| 1 | **야만인 (Barbarian)** | 이벤트로 컬렉션에 추가 | 인접 아군 심볼 식량 생산 -50% | 6 / 1 |
...
- 모든 적 심볼은 컬렉션에 추가되며, 배치 우선순위 규칙에 따라 거의 매 스핀 보드에 등장.
- 적 심볼의 HP는 `effect_counter`로 추적. 보드 위에 잔여 HP 숫자 표시 필요.
