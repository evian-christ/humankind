...
| 계층 | 이름 | 등장 방식 | 효과 | 제거 조건 |
|------|------|----------|------|----------|
| 1 | **야만인 (Barbarian)** | 이벤트로 보드에 강제 배치 | 인접 심볼 식량 생산 -50% 또는 매 턴 -3 Food | Warrior급 전투 심볼 인접 |
| 2 | **침략자 (Raider)** | 전쟁 심볼이 컬렉션에 추가 | 매 턴 인접 아군 심볼 1개 파괴 시도 | Swordsman급 이상 |
| 3 | **정예 적군 (Elite Enemy)** | 고위 전쟁 심볼이 컬렉션에 추가 | 인접 아군 파괴 + 주변 2칸 식량 생산 차단 | 고위 전투 심볼만 가능 |

- 야만인은 이벤트로 보드에 직접 배치 (컬렉션 추가 아님).
- 침략자·정예 적군은 컬렉션에 추가되므로 이후 스핀에서 보드에 랜덤 등장.

### 6-3. 전쟁 심볼 (플레이어 선택 가능)
